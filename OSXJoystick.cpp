...
namespace OSX
{
  ...
  std::vector<OSX::Joystick*> allJoysticksVector;
  ...
  std::string joystickName(int idxOfWhich)
  {
    ...
    if(idxOfWhich < 0 || idxOfWhich > OSX::joystickCount())
    {
      ::fprintf(stderr, "there is no joystick number %d", idxOfWhich);
      return std::string("");
    }
    int index = idxOfWhich;
    recDevice *device = gpDeviceList;
    
    for (; index > 0; index--)
      device = device->pNext;
    
    std::string strResult = device->product;
    return strResult;
  }
  ...
  OSX::Joystick* connectToJoystick(int idxOfWhich)
  {
    ...
    OSX::Joystick* j = new OSX::Joystick;
    j->iD = idxOfWhich;
    recDevice *device = gpDeviceList;
    ...
    j->name = device->product;
    j->numberOfAxis = device->axes;
    j->numberOfHats = device->hats;
    j->numberOfBalls = 0;
    j->numberOfButtons = device->buttons;
    
    j->axisDataVector.resize(j->numberOfAxis);
    j->buttonDataVector.resize(j->numberOfButtons);
    j->hatDataPositionVector.resize(j->numberOfHats);
    return j;
  }
  ...
  void Joystick::update()
  {
    ...
    for(int index = iD; index > 0; index--)
      device = device->pNext;
    ...
    {     
      if (device->uncentered) 
      {
        ...
        for (i = 0; i < device->axes; i++)
          axisDataVector[i] = 0;//SDL_PrivateJoystickAxis(joystick, i, 0);
        
        for (i = 0; i < device->buttons; i++)
          buttonDataVector[i] = false; //SDL_PrivateJoystickButton(joystick, i, 0);
        
        for (i = 0; i < device->hats; i++)
          hatDataPositionVector[i] = eHatCenter; //  SDL_PrivateJoystickHat(joystick, i, SDL_HAT_CENTERED);
    ...
    else
    {
      ...
      while (element) 
      {
        ...
        if (value >= 1)          /* handle pressure-sensitive buttons */
          buttonDataVector[i] = true;
        else
          buttonDataVector[i] = false;
